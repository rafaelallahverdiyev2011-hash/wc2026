...
# Fix the broken comment in HARDCODED_RESULTS
old = """  "Ghana_Colombia":                   { home: 0, away: 1 },
  // Group stage (legacy){
...
new = """  "Ghana_Colombia":                   { home: 0, away: 1 },
  // Group stage (legacy)
  "Mexico_South Africa": { home: 2, away: 0 },"""

if old in content:
    content = content.replace(old, new)
    print('Step 1: Fixed duplicate entries')
else:
    print('ERROR step 1: marker not found')

# Now apply HARDCODED_RESULTS in KnockoutTab static fixture mapping
old2 = "        score: { winner: null, duration: 'REGULAR' as const, fullTime: { home: null, away: null }, halfTime: { home: null, away: null } },"
new2 = """        score: (() => {
          const key = `${f.home}_${f.away}`;
          const hc = HARDCODED_RESULTS[key];
...
if old2 in content:
    ...
    print('Step 2: Applied HARDCODED_RESULTS to static fixtures')
else:
